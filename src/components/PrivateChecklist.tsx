    ...
    return (
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          This section is private. Unlock to view your daily checklist.
        </p>
        <Link
          to="/unlock"
                ...
                {grouped[t].map((it) => (
                  <li key={it.id}>
                    <label className="flex items-center gap-3 rounded-md p-2 hover:bg-secondary/50 cursor-pointer">
                      ...
                      />
                      <div
                        className={`flex-1 ${checked[it.id] ? "line-through text-muted-foreground" : ""}`}
                      >
                        <div className="text-sm font-medium">
                          {it.name} <span className="text-muted-foreground">· {it.dose}</span>
                        </div>
                        {it.notes && (
